...
const Dynamic2 = () => {
  ...
  const { url3 } = useContext(UserContext);
  ...
  const Loading = async () => {
    try {
      const res = await axios.get(`${url3}/${params.id}`);
      setItem(res.data);
      console.log(res);
    } catch (error) {
  ...
  return (
    <div className="container">
      {toArr.length === 0 && <h1 className="absolute text-4xl top-0 left-0 bottom-0 right-0 w-full opacity-90 flex items-center justify-center bg-gray-200  text-white">LOADING...</h1>}
      {getError.length > 0 ? (
